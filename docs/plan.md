...
## 九、GraphRAG 增强执行进展（2026-03-13）

### 已实施项

| 模块 | 实施内容 | 结果 |
|------|----------|------|
| 全量索引 | 新增 `BatchIndexer`，支持增量跳过与并发索引 | ✅ 已实现 |
| LLM Prompt | 增加类名、层级、字段依赖上下文 | ✅ 已实现 |
| 向量库能力 | `KnowledgeBase` 增加 `exists/count/search` | ✅ 已实现 |
| 主流程参数 | `main.py` 新增 `--index-all`，支持全量索引模式 | ✅ 已实现 |
